...
def render_sidebar():
    with st.sidebar:
        ...
        if 'user' in st.session_state:
            ...
            if st.button("Déconnexion"): st.session_state.pop('auth_token', None); st.rerun()
        else: menu = "Login"
        st.markdown("---")
...
def render_audit_page():
    ...
    if hours > 0:
        ...
        if next_maint < 50: st.markdown(f"""<div class="maintenance-alert">🛠️ Vidange dans {next_maint:.0f}h.</div>""", unsafe_allow_html=True)
    ...
    if st.button("LANCER L'AUDIT", type="primary", disabled=blocked):
        if hours <= 0: st.error("Index incohérents.")
            ...
            with st.spinner("Calcul..."):
                ...
                st.session_state['last_audit'] = {'eq_id': selected_id, 'eq_name': eq_data['equipment_name'], 'scenario': scenario_code, 'start': start_h, 'end': end_h, 'fuel': fuel_l, 'est': est_fuel, 'dev': dev, 'z': anom.z_score, 'verdict': anom.verdict, 'conf': anom.confidence, 'hours': hours, 'src': src}
    ...
    if 'last_audit' in st.session_state:
        ...
        with c_share:
            ...
            st.markdown(f'<a href="https://wa.me/?text={urllib.parse.quote(msg_wa)}" target="_blank" class="share-btn">📲 WhatsApp</a>', unsafe_allow_html=True)
...
def render_calibration_page():
    ...
    st.info("ℹ️ Les profils constructeurs verrouillent la puissance pour garantir la précision. Utilisez 'GÉNÉRIQUE' pour une saisie libre.")
    ...
    user_pwr = c3.number_input(f"Puissance Nominale ({unit_label})", value=float(display_val), disabled=not is_generic, help="Verrouillé pour les profils constructeurs.")
    ...
    if st.button("ENREGISTRER LA CALIBRATION"):
        if eid and name:
            ...
            try:
                st.session_state.db.execute_write("INSERT INTO equipment (equipment_id, equipment_name, profile_base, power_kw) VALUES (?, ?, ?, ?)", (eid, name, code, final_kw))
                st.success(f"✅ {name} Calibré"); time.sleep(1); st.rerun()
            except: st.error("ID existant.")
        else: st.warning("ID et Nom requis.")
...
def render_learning_page():
    ...
    if st.session_state.get('license_tier') == 'CORPORATE':
        if st.button("Lancer Apprentissage"): st.session_state.learning.batch_learn_from_all_equipment(st.session_state.db); st.success("OK")
    else: st.warning("Réservé CORPORATE")

def render_admin_page():
    if st.session_state.get('role') != 'admin': return
    st.markdown('<div class="main-header">🔐 Admin QG</div>', unsafe_allow_html=True)
    t1, t2, t3, t4 = st.tabs(["⚙️ Config Globale", "💰 Paiements", "👥 Utilisateurs", "💾 Maintenance"])
    
    with t1:
        st.subheader("Paramètres Globaux (Modèle Physique)")
        current_aging = st.session_state.db.get_config_value("AGING_FACTOR", "1.05")
        c1, c2 = st.columns([3, 1])
        new_aging = c1.slider("Facteur Vieillissement / Tropicalisation", 1.0, 1.3, float(current_aging), 0.01)
